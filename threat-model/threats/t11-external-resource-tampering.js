    ...
    "response": [
      {
        ...
        "type": "Reduce",
        "desc": "[=E1|List publishers=] use content integrity protection (such as <code>digestMultibase</code>) for all externally-referenced resources in [=O1|recognized entity credentials=]. Because content integrity verification detects substitution regardless of whether the attack is a server compromise, domain takeover, or DNS hijack, it is the primary defense against all three mechanisms. [=E4|Verifiers=] verify the integrity of any fetched resource against the declared digest before applying it during [=P6|Validate Credential(s)=], and treat a digest mismatch as a hard failure. [=E4|Verifiers=] use DNSSEC-validating resolvers and verify TLS certificates against expected values when fetching external resources. Where possible, schema files are embedded directly in the credential or hosted on infrastructure under the [=E1|list publisher=]\u2019s direct control within the [=C1|publisher system=]. Any [=O1|recognized entity credential=] that references external resources without integrity protection is rejected by conforming [=E4|verifiers=]."
      }
    ],
