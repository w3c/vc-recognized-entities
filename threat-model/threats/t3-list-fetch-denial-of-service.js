    ...
    "response": [
      {
        ...
        "type": "Reduce",
        "desc": "[=E3|Holders=] SHOULD present [=O1|recognized entity credentials=] directly to [=E4|verifiers=] (\u201ccredential stapling\u201d) rather than requiring [=E4|verifiers=] to fetch them, distributing load away from the list host. When direct fetching is necessary, [=E4|verifier=] implementations MUST enforce both connection and read timeouts and MUST enforce maximum payload size limits on all list fetches. [=E4|Verifiers=] MUST also impose maximum list size thresholds and SHOULD reject or skip lists that exceed configurable limits. Implementations SHOULD use streaming or paginated access patterns rather than loading entire lists into memory. [=E4|Verifiers=] SHOULD cache resolved [=O1|recognized entity credentials=] with appropriate TTLs aligned to the credential\u2019s <code>validUntil</code> period. [=E1|List publishers=] SHOULD deploy content delivery network infrastructure, rate limiting, and paginated or indexed lookup mechanisms to allow [=E4|verifiers=] to check membership without retrieving full lists."
      }
    ],
