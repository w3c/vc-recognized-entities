    ...
    "response": [
      {
        ...
        "type": "Reduce",
        "desc": "[=E1|List publishers=] SHOULD select validity periods appropriate to the risk profile and operational capacity of the ecosystem. High-risk or large ecosystems should prefer short validity periods (24\u201348 hours); smaller, lower-risk ecosystems may use longer periods provided robust revocation mechanisms are also deployed. [=E1|List publishers=] SHOULD provide revocation status endpoints that allow point-in-time queries rather than requiring full list re-fetches. [=E4|Verifier=] implementations MUST check the <code>validUntil</code> field during [=P6|Validate Credential(s)=] and MUST NOT accept expired credentials. [=E4|Verifiers=] MUST respect cache-control directives from [=E1|list publishers=] and SHOULD enforce maximum cache TTLs appropriate to the ecosystem risk profile, performing fresh retrieval or live revocation checks when cached data exceeds the configured staleness threshold. Governance frameworks SHOULD specify acceptable cache freshness windows for compliant [=E4|verifier=] implementations."
      }
    ],
